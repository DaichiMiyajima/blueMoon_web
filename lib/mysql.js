...
mysql.insertMyWork = function (workName,description,PicturePath,cb){
var result = {};
	var insert_myWork_table = connection.query(
			'INSERT INTO myWork(name, description,picturePath) VALUE(?,?,?)'
			,[workName,description,PicturePath]
...
mysql.selectAttendanceMessage = function (callback){
	var data = [];
	var worl_select = connection.query("SELECT ID,name,description,picturePath,date FROM blueMoon.myWork order by date desc",function (error, results, fields) {
		if (error) throw err;
